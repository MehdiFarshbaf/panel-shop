...
// ** Utils
import {handleShowErrorMessage, isUserLoggedIn} from '@utils'
...
const ForgotPassword = () => {
    ...
    const formik = useFormik({
        ...
        onSubmit: async values => {
            try {
                setLoading(true)
                const {data} = await API.post("/admin/forgot-password", values)
                if (data.success) {
                    toast.success(data.message)
                    navigation("/reset-password", {state: {email: values.email}})
                    ...
                    <Col className='d-flex align-items-center auth-bg px-2 p-lg-5' lg='4' sm='12'>
                        <Col className='px-xl-2 mx-auto' sm='8' md='6' lg='12'>
                            ...
                            <CardText className='mb-2'>
                                ایمیل خود را وارد کنید و ما دستورالعمل هایی را برای بازنشانی رمز عبور برای شما ارسال می
                                کنیم
                            </CardText>
                            <Form className='auth-forgot-password-form mt-2' onSubmit={formik.handleSubmit}>
                                <div className='mb-1'>
                                ...
                                <Button.Ripple color='primary' type={"submit"} disable={loading} block>
                                    {loading ? <Spinner color='white' size='sm'/> : "ارسال ایمیل"}
                                </Button.Ripple>
                            </Form>
