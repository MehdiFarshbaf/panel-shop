...
const EditCard = ({ currentData, categories, categoriesArray, forms }) => {
   ...
   const schema = yup.object().shape({
      options: yup.array().of(
           yup.object().shape({
              name: yup
                   .string()
                   .matches(/^[a-zA-Z][a-zA-Z0-9]*$/, 'نام باید انگلیسی باشد و به صورت camelCase')
                   .required('نام الزامی است')
           })
      )
   })
   const defaultValues = {
      title             : currentData._source?.items?.form.description?.title,
      description       : currentData._source?.items?.form.description?.description,
      image             : currentData._source?.items?.form.description?.image,
      ...(languages.length >= 1 && {
         ...languages.reduce((acc, language) => {
            acc[`title_${ language }`] = currentData._source?.items?.form.description[`title_${ language }`]
            return acc
         }, {})
      }),
      ...(languages.length >= 1 && {
         ...languages.reduce((acc, language) => {
            acc[`description_${ language }`] = currentData._source?.items.form.description[`description_${ language }`]
            return acc
         }, {})
      }),
      ...(languages.length >= 1 && {
         ...languages.reduce((acc, language) => {
            acc[`image_${ language }`] = currentData._source?.items?.form.description[`image_${ language }`]
            return acc
         }, {})
      }),
      activeMap         : currentData._source?.items?.form.setting.map.active,
      hideExactLocation : currentData._source?.items?.form.setting.map.hideExactLocation,
      installment : currentData._source?.items?.form.setting.installment,
      deposit : currentData._source?.items?.form.setting.deposit,
      exchange : currentData._source?.items?.form.setting.exchange,
      consensualAd : currentData._source?.items?.form.setting.consensualAd,
      activePrice : currentData._source?.items?.form.setting.activePrice,
      
      activeVirtualTour : currentData._source?.items?.form.setting.virtualTour.active,
      activeImage       : currentData._source?.items?.form.setting.image.active,
      max               : currentData._source?.items?.form.setting.image.max,
      category          : {
         label : currentData._source?.items?.category,
         value : currentData._source?.items?.category
      },
      innerCategory     : {
         label : currentData._source?.items?.innerCategory,
         value : currentData._source?.items?.innerCategory
      },
      subCategory       : {
         label : currentData._source?.items?.subCategory,
         value : currentData._source?.items?.subCategory
      },
      options           : currentData._source?.items?.form.options.map(option => {
         return {
            title       : option.title,
            ...(languages.length >= 1 && {
               ...languages.reduce((acc, language) => {
                  acc[`title_${ language }`] = option[`title_${ language }`]
                  return acc
               }, {})
            }),
            type        : { value : option.type, label : option.type },
            name        : option.name,
            required    : option.required,
            description : option.description,
            ...(languages.length >= 1 && {
               ...languages.reduce((acc, language) => {
                  acc[`description_${ language }`] = option[`description_${ language }`]
                  return acc
               }, {})
            }),
            ...(languages.length >= 1 && {
               ...languages.reduce((acc, language) => {
                  acc[`placeholder_${ language }`] = option[`placeholder_${ language }`]
                  return acc
               }, {})
            }),
            placeholder : option.placeholder,
            activatedBy : option.activatedBy && option.activatedBy !== null &&
            option.activatedBy.length >= 1 ? option.activatedBy.map(o => ({
               value : o,
               label : o
            })) : null,
            ...(option.type === "text" && {
               ...
               ...(languages.length >= 1 && {
                  ...languages.reduce((acc, language) => {
                     acc[`append_${ language }`] = option[`append_${ language }`]
                     return acc
                  }, {})
               }),
            ...
            ...(option.type === "select" && {
               multilineSelect : option.multiline
               
            }),
            ...(option.type === "select" && option.options && option.options.length >= 1 && {
               nestedArray : option.options.map((opt, index) => ({
                  label : opt.label,
                  ...(languages.length >= 1 && {
                     ...languages.reduce((acc, language) => {
                        acc[`label_${language}`] = option.options[index][`label_${language}`]
                        return acc
                     }, {})
                  }),
                  value : opt.value
            ...
            ...(option.type === "limited_select" && {
               multilineSelect : option.multiline
            }),
            ...(option.type === "limited_select" && option.options && option.options.length >= 1 && {
               nestedArray : option.options.map((opt, index) => ({
                  label : opt.label,
                  ...(languages.length >= 1 && {
                     ...languages.reduce((acc, language) => {
            ...
            ...(option.type === "range" && {
               placeholderRange : option.placeholder,
               ...(languages.length >= 1 && {
                  ...languages.reduce((acc, language) => {
                     acc[`placeholderRange_${language}`] = option[`placeholder_${language}`]
                     return acc
                  }, {})
               }),
               ...(languages.length >= 1 && {
                  ...languages.reduce((acc, language) => {
                     acc[`titleRange_${language}`] = option[`title_${language}`]
                     return acc
                  }, {})
               }),
               titleRange       : option.title,
               numericRange     : option.numeric
            }),
            ...(option.type === "range" && option.from.length >= 1 && {
               nestedArrayFrom : option.from.map((opt, index) => ({
                  label : opt.label,
                  ...(languages.length >= 1 && {
                     ...languages.reduce((acc, language) => {
                        acc[`label_${language}`] = option.from[index][`label_${language}`]
                        return acc
                     }, {})
                  }),
                  value : opt.value
            ...
            ...(option.type === "range" && option.to.length >= 1 && {
               nestedArrayTo : option.to.map((opt, index) => ({
                  label : opt.label,
                  ...(languages.length >= 1 && {
                     ...languages.reduce((acc, language) => {
                        acc[`label_${language}`] = option.to[index][`label_${language}`]
                        return acc
                     }, {})
                  }),
                  value : opt.value
   ...
   const onSubmit = data => {
      setIsLoading(true)
      const {
               category, innerCategory, subCategory, title, title_en, description, description_en, image, image_en
      ...
      axios.post(`Ads/forms/update?id=${ currentData._id }`, {
         ...(innerCategory.value !== undefined ? { cat_name : innerCategory.value } : subCategory.value !== undefined && { cat_name : subCategory.value }),
         items : {
               ...
               options     : newOptions.map((option, index) => ({
                  ...
                  ...(option.type.value === "limited_select" && option.nestedArray.length >= 1 && {
                     options : option.nestedArray.map((opt, i) => ({
                        ...
                        ...(languages.length >= 1 && {
                           ...languages.reduce((acc, language) => {
                              acc[`label_${language}`] = newOptions[index].nestedArray[i][`label_${language}`]
                              return acc
                           }, {})
                        }),
                        value : opt.value
                  ...
                  ...(option.type.value === "range" && {
                     ...
                     ...(languages.length >= 1 && {
                        ...languages.reduce((acc, language) => {
                           acc[`placeholder_${language}`] = newOptions[index][`placeholderRange_${language}`]
                           return acc
                        }, {})
                     }),
                     ...
                     ...(languages.length >= 1 && {
                        ...languages.reduce((acc, language) => {
                           acc[`title_${language}`] = newOptions[index][`titleRange_${language}`]
                           return acc
                        }, {})
                     }),
                  ...
                  ...(option.type.value === "range" && option.nestedArrayFrom.length >= 1 && {
                     from : option.nestedArrayFrom.map((opt, i) => ({
                        ...
                        ...(languages.length >= 1 && {
                           ...languages.reduce((acc, language) => {
                              acc[`label_${language}`] = newOptions[index].nestedArrayFrom[i][`label_${language}`]
                              return acc
                           }, {})
                        }),
                        value : opt.value,
                  ...
                  ...(option.type.value === "range" && option.nestedArrayTo.length >= 1 && {
                     to : option.nestedArrayTo.map((opt, i) => ({
                        ...
                        ...(languages.length >= 1 && {
                           ...languages.reduce((acc, language) => {
                              acc[`label_${language}`] = newOptions[index].nestedArrayTo[i][`label_${language}`]
