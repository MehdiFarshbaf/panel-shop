...
// ** Table Header
const CustomHeader = ({ store,isExcelLoading, toggleSidebar, setDownloadExcel, downloadExcel, handlePerPage, rowsPerPage, handleFilter, searchTerm }) => {
      ...
      <Row>
        <Col xl='6' className='d-flex align-items-center p-0 px-lg-1'>
           ...
           <div className='d-flex align-items-center table-header-actions'>
              <Button className='add-new-user' outline color='success' onClick={() => setDownloadExcel(true)}>
                 <Grid className='font-small-4 me-50' />
                 <span className='align-middle'>{isExcelLoading ? "در حال بازگذاری" : "خروجی اکسل" }</span>
              </Button>
  ...
  useEffect(() => {
   ...
   useEffect(() => {
      if(downloadExcel){
         ...
         axios({
            url: "https://back.dastrass.ir/api/admin/transaction/excel",
            method: "GET",
            responseType: "blob",
            headers: {
            ...
            downloadLink.setAttribute("download", "transaction.xlsx")
            ...
            toast.error("خطا در دریافت خروجی اکسبل.")
            setIsExcelLoading(false)
         })
      }
