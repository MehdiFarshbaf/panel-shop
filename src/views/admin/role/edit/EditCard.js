...
const EditCard = ({currentData, permissionsData }) => {
   ...
   const defaultValues = {
      ...
      permissions         : currentData.permissions.map(p => ({value:p.id , label:p.title}))
   ...
   const onSubmit = data => {
      ...
      axios.post(`/admin/role/${currentData.id}`, dataInfo).then(res => {
              if (res.status === 200) {
              ...
              <CardBody className = "invoice-padding pt-0" >
                 <Form className = "mt-2 pt-50" onSubmit = { handleSubmit(onSubmit) } >
                    ...
                    <Row >
                       ...
                       {
                            data?.role.permissions?.find(permission => permission.key.trim() === "ROLES_UPDATE".trim()) &&
